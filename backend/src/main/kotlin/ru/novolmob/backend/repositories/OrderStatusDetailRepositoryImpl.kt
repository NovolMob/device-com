...
import ru.novolmob.backend.exceptions.orderStatusDetailByDeviceIdAndLanguageNotFound
import ru.novolmob.backend.exceptions.orderStatusDetailByIdNotFound
...
class OrderStatusDetailRepositoryImpl(
    ...
    override suspend fun getDetailFor(
        ...
        newSuspendedTransaction(Dispatchers.IO) {
            OrderStatusDetail
                ...
                .limit(1).firstOrNull()?.let(mapper::invoke) ?: orderStatusDetailByDeviceIdAndLanguageNotFound(orderStatusId, language).left()
        }
    ...
    override suspend fun get(id: OrderStatusDetailId): Either<BackendException, OrderStatusDetailModel> =
        newSuspendedTransaction(Dispatchers.IO) {
            OrderStatusDetail.findById(id)?.let(mapper::invoke) ?: orderStatusDetailByIdNotFound(id).left()
        }
    ...
    override suspend fun post(createModel: OrderStatusDetailCreateModel): Either<BackendException, OrderStatusDetailModel> =
        newSuspendedTransaction(Dispatchers.IO) {
            val orderStatus = OrderStatus.findById(createModel.orderStatusId) ?: return@newSuspendedTransaction orderStatusByIdNotFound(createModel.orderStatusId).left()
    ...
    override suspend fun post(
        ...
        newSuspendedTransaction(Dispatchers.IO) {
            ...
            }?.let(mapper::invoke) ?: orderStatusDetailByIdNotFound(id).left()
        }
