...
import ru.novolmob.backend.exceptions.deviceDetailByDeviceIdAndLanguageNotFound
import ru.novolmob.backend.exceptions.deviceDetailByIdNotFound
...
class DeviceDetailRepositoryImpl(
    ...
    override suspend fun get(id: DeviceDetailId): Either<BackendException, DeviceDetailModel> =
        newSuspendedTransaction(Dispatchers.IO) {
            DeviceDetail.findById(id)?.let(mapper::invoke) ?: deviceDetailByIdNotFound(id).left()
        }
